...
import type { LlmMessage } from "./ollamaClient";
...
export interface OllamaVisionMessage extends LlmMessage {
 ...
 */
export async function prepareMessagesWithVision(
  messages: LlmMessage[],
  model: string,
  ollamaUrl: string,
  ...
  const visionMessages = messages as OllamaVisionMessage[];
  ...
  output.appendLine(
    `[vision] Model ${model} does not support vision. Enriching messages with extracted screenshot state.`,
  );

  let visionModel: string | null = configuredVisionModel?.trim() || null;
  ...
  const processed: OllamaVisionMessage[] = [];
    ...
    if (visionModel) {
      ...
      for (const img of msg.images) {
        try {
          ...
          if (visionResp.ok) {
            const visionData = (await visionResp.json()) as { response: string };
