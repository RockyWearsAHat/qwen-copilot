...
import { LlmMessage } from "../../ollamaClient";
...
export class MessageConverter {
  private static readonly maxToolResultChars = 1400;
  private static readonly maxDataPartChars = 6000;
  ...
  convertRequestMessages(
    ...
    for (const message of messages) {
      ...
      for (const part of message.content) {
        ...
        if (part instanceof vscode.LanguageModelToolResultPart) {
          const toolContent = part.content
            .map((resultPart) =>
              resultPart instanceof vscode.LanguageModelTextPart ? resultPart.value : "",
            )
            .filter((entry) => entry.length > 0)
            .join("\n");

          const content =
            toolContent.length <= MessageConverter.maxToolResultChars
              ? toolContent
      ...
      const shouldEmitBaseMessage =
        content.length > 0 || images.length > 0 || assistantToolCalls.length > 0;
  ...
  mapMessageRole(role: vscode.LanguageModelChatMessageRole): LlmMessage["role"] {
  ...
  sanitizeCopilotPreambleMessage(
    content: string,
  ...
  minimizeCopilotPreamble(content: string): string {
  ...
  compactEnvelopeUserMessage(content: string): string {
    ...
    if (extractedUserRequest) {
      ...
      const parts: string[] = [extractedUserRequest.trim()];
      if (context) {
  ...
  looksLikeCopilotPreamble(content: string): boolean {
  ...
  extractTaggedSection(content: string, tag: string): string {
  ...
  compactCopilotPreambleContent(content: string): string {
    let result = content;
  ...
  partToText(part: unknown): string {
    ...
    if (part && typeof part === "object") {
      const candidate = part as { mimeType?: unknown; data?: unknown; value?: unknown };
  ...
  private decodeDataPartPayload(payload: unknown, mimeType: string): string {
    if (!payload) return "";
  ...
  private tryDecodeUtf8(payload: unknown): string {
    try {
  ...
  extractImageBase64(part: unknown): string | undefined {
    if (!part || typeof part !== "object") {
      return undefined;
    }
  ...
  toBase64(payload: unknown): string | undefined {
    if (payload instanceof Uint8Array) {
    ...
    if (
      ...
      return Buffer.from((payload as { data: number[] }).data).toString("base64");
    }

    return undefined;
  }

  isDebugDumpEnabled(): boolean {
    return process.env.LOCAL_QWEN_DEBUG_DUMP === "1";
  }

  writeDebugDump(filePath: string, payload: string, summary: string): void {
    void writeFile(filePath, payload, "utf8")
      .then(() => {
