...
import { isMachineInteractionToolName } from "./machineInteractionPolicy";
 ...
 */
const TOOL_SCHEMA_MAP: Readonly<
  Record<string, { description: string; parameters: Record<string, unknown> }>
> = {
  read_file: {
    description:
      "Read a range of lines from a file inside the workspace. Use this to inspect file contents before editing. Prefer reading broad ranges once over many small reads.",
    parameters: {
      type: "object",
      required: ["filePath"],
      properties: {
  ...
  write_file: {
    description:
      "Create or fully overwrite a file with the given content. Use for new files or wholesale replacements. Prefer edit_file for surgical changes to existing files.",
    parameters: {
      type: "object",
  ...
  edit_file: {
    description:
      "Replace a specific line range inside an existing file. Provide the exact lines to replace and the new text. Both startLine and endLine are 1-based and inclusive. Use this instead of write_file when modifying only part of a file.",
    parameters: {
      type: "object",
      ...
      properties: {
        ...
        newText: {
          type: "string",
          description:
            "Replacement text (may span multiple lines). An empty string deletes the range.",
        },
      },
      additionalProperties: false,
    },
  ...
  grep_search: {
    description:
      "Search for a text pattern or regex within workspace files. Returns file paths and matching lines. Use to locate symbol definitions, usages, or specific strings without reading every file.",
    parameters: {
      type: "object",
      required: ["query"],
      properties: {
  ...
  replace_in_files: {
    description:
      "Replace all occurrences of a string across multiple files matching a glob pattern. Use for bulk renames or consistent string substitutions. Returns the count of replacements made.",
    parameters: {
      type: "object",
      required: ["from", "to"],
      properties: {
  ...
  run_in_terminal: {
    description:
      "Execute a shell command for build, test, install, or runtime tasks. " +
    ...
    parameters: {
      ...
      properties: {
        ...
        showTerminal: {
          type: "boolean",
          description:
            "When true, run command in a visible VS Code terminal instead of hidden child process execution (default: false).",
        },
      },
      additionalProperties: false,
    },
  ...
  get_terminal_output: {
    description:
      "Retrieve buffered output from a background terminal process previously started with run_in_terminal. Use to check if a background build or server has finished or produced errors.",
    parameters: {
      type: "object",
      required: ["id"],
      properties: {
  ...
  kill_terminal: {
    description:
      "Terminate a background terminal process previously started with run_in_terminal. Call this to clean up after a background server or watcher is no longer needed.",
    parameters: {
      type: "object",
      required: ["id"],
      properties: {
    ...
    parameters: {
      ...
      properties: {
        title: {
          type: "string",
          description:
            "Short descriptive title for the work plan (e.g. '3D Parkour Game Implementation').",
        },
        items: {
          type: "array",
          items: { type: "string" },
          description:
            "Ordered list of specific, concrete sub-tasks covering the full request scope. NOT high-level milestones (those go in agent_progress). Each item should map to one or a few related file changes.",
        },
      },
      additionalProperties: false,
    },
  ...
  get_agent_checklist: {
    description:
      "Read and parse .github/agent-checklist.md — the agent's private internal work plan. Call regularly to see which sub-tasks are still pending. Returns checked/unchecked items with a progress summary.",
    parameters: {
      type: "object",
      properties: {},
      additionalProperties: false,
  ...
  update_agent_checklist_item: {
    description:
      "Mark a specific item in .github/agent-checklist.md as done or not done. Call after completing each sub-task. Matched by partial text (case-insensitive).",
    parameters: {
      type: "object",
  ...
  take_screenshot: {
    description:
      "Capture a screenshot of the screen or a specific window/region, returned as base64 PNG. Use to visually verify GUI apps, websites, desktop apps. On macOS uses screencapture, on Linux uses scrot/gnome-screenshot. Also returns best-effort metadata (origin/size) to help map OCR results to absolute screen coordinates.",
    parameters: {
      type: "object",
      properties: {
        windowTitle: {
  ...
  ocr_find_text: {
    description:
      "Run local OCR over a screenshot image and return bounding boxes + coordinates for matching text. Use this to turn 'click the \"OK\" button' into pixel coordinates before calling gui_click. Requires tesseract to be installed on the host.",
    parameters: {
      type: "object",
      ...
      properties: {
        ...
        origin: {
          type: "object",
          description:
            "Optional absolute screen origin offset (x,y) for the screenshot. When provided, returns absolute click coordinates in addition to image-local coordinates.",
          properties: {
            x: { type: "number" },
            y: { type: "number" },
          },
  ...
  analyze_image: {
    description:
      "Send an image (base64 or file path) to the Ollama model for visual analysis. Use to understand screenshots, identify UI elements, read text from images. Returns the model's textual analysis.",
    parameters: {
      type: "object",
      required: ["image"],
      properties: {
  ...
  gui_click: {
    description:
      "Simulate a mouse click at screen coordinates. Use after take_screenshot to interact with GUI apps, browsers, desktop apps.",
    parameters: {
      type: "object",
      required: ["x", "y"],
      properties: {
  ...
  gui_type: {
    description:
      "Simulate keyboard text input. Types text as if user typed it. Use after gui_click to fill forms or type commands.",
    parameters: {
      type: "object",
      required: ["text"],
      properties: {
  ...
  gui_scroll: {
    description:
      "Simulate mouse scroll at screen coordinates. Use for scrolling web pages, documents, or scrollable UI elements.",
    parameters: {
      type: "object",
  ...
  gui_key: {
    description:
      "Press a key or key combination. Use for shortcuts (Cmd+Q, Ctrl+C, Escape, Enter, etc.) during GUI testing.",
    parameters: {
      type: "object",
      required: ["key"],
      properties: {
        key: {
          type: "string",
          description:
            "Key name: 'enter', 'escape', 'tab', 'space', 'up', 'down', 'left', 'right', 'backspace', 'delete', 'f1'-'f12', or any character.",
        },
        modifiers: {
          type: "array",
          items: { type: "string", enum: ["cmd", "ctrl", "alt", "shift"] },
  ...
  gui_key_hold: {
    description:
      "Press and hold a key for a short duration then release. Mainly useful for modifier keys during GUI automation. On macOS this is best-effort and primarily supports modifiers via cliclick.",
    parameters: {
      type: "object",
      required: ["key"],
      properties: {
  ...
  list_windows: {
    description:
      "List all visible application windows. Returns titles, app names, positions, sizes. Use to find windows to screenshot or interact with.",
    parameters: {
      type: "object",
      properties: {
  ...
  focus_window: {
    description:
      "Bring a window to the foreground. Use before gui_click/gui_type to ensure correct window gets input.",
    parameters: {
      type: "object",
  ...
  launch_app: {
    description:
      "Launch an application or URL. Use to start the app-under-test before GUI testing. On macOS uses 'open', on Linux uses 'xdg-open'.",
    parameters: {
      type: "object",
      required: ["target"],
      properties: {
  ...
  wait_for_condition: {
    description:
      "Poll until a condition is met: file exists, port listening, process running, or text on screen. Use to sync between launching an app and interacting with it.",
    parameters: {
      type: "object",
      required: ["type"],
      properties: {
  ...
  get_diagnostics: {
    description:
      "Get VS Code diagnostics (errors, warnings) for a file or the entire workspace. Use after code changes to verify zero errors.",
    parameters: {
      type: "object",
      properties: {
  ...
  get_workspace_symbols: {
    description:
      "Search for symbols (functions, classes, variables, types) across the workspace by name. Use to quickly locate definitions.",
    parameters: {
      type: "object",
      required: ["query"],
      properties: {
  ...
  get_document_symbols: {
    description:
      "Get all symbols in a specific file — functions, classes, methods, imports. Use to understand file structure before editing.",
    parameters: {
      type: "object",
      required: ["filePath"],
      properties: {
  ...
  get_references: {
    description:
      "Find all references to a symbol at a file location. Use to understand impact before modifying a function/class/variable.",
    parameters: {
      type: "object",
  ...
  get_definition: {
    description:
      "Jump to a symbol's definition. Returns file and line where it is defined. Use to trace imports and declarations.",
    parameters: {
      type: "object",
      required: ["filePath", "line", "character"],
      properties: {
  ...
  http_request: {
    description:
      "Make an HTTP request and return the response. Use to test REST APIs, verify servers, or fetch data.",
    parameters: {
      type: "object",
      required: ["url"],
      properties: {
  ...
  agent_progress: {
    description:
      "Report USER-FACING high-level progress milestones. This is the user's window into what the agent is doing — keep items high-level (5–10 across the whole task). Do NOT call this after every checklist sub-task; call it only when a major logical section of work is complete (e.g. 'scaffold complete', 'player controller written', 'physics integrated'). The detailed internal tracking lives in the completion checklist — this is the executive summary for the user.",
    parameters: {
      type: "object",
...
export class ToolRegistry {
  ...
  public async execute(name: string, args: Record<string, unknown>): Promise<unknown> {
    ...
    if (!machineToolsEnabled && isMachineInteractionToolName(name)) {
      return {
        success: false,
