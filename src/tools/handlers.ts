...
import { getTerminalPolicyViolation } from "./terminalPolicy";
...
function boundsArea(bounds?: { width: number; height: number }): number {
  if (!bounds) return 0;
...
export function __selectBestMacWindowForTest(
  windows: MacWindowInfo[],
  needleRaw: string,
): MacWindowInfo | undefined {
  ...
  const score = (w: MacWindowInfo): number => {
    const app = normalizeNeedle(w.app ?? "");
    const title = normalizeNeedle(w.title ?? "");

    let s = 0;
    if (title === needle) s += 1200;
    if (app === needle) s += 900;
    if (title.includes(needle)) s += 700;
    if (app.includes(needle)) s += 450;
    if (title.length > 0) s += 50;
    s += Math.min(400, Math.round(boundsArea(w.bounds) / 5000));
    return s;
  };

  let best: MacWindowInfo | undefined;
  let bestScore = -1;
  ...
  if (!best || bestScore < 450) return undefined;
  return best;
}

async function ensureMacWindowInfoBinary(): Promise<string> {
  ...
  try {
    ...
    if (current !== MAC_WINDOW_INFO_SWIFT) needsWrite = true;
  } catch {
    needsWrite = true;
  }
  if (needsWrite) {
...
async function listMacWindowsViaSwift(): Promise<MacWindowInfo[]> {
  ...
  return (parsed as MacWindowInfo[]).filter((w) => typeof w?.id === "number");
}

async function listMacWindowsViaYabai(): Promise<MacWindowInfo[]> {
  const { stdout } = await execFileAsync("yabai", ["-m", "query", "--windows"], {
    maxBuffer: 5 * 1024 * 1024,
  });
  const parsed = JSON.parse(String(stdout ?? "")) as Array<any>;
  if (!Array.isArray(parsed)) return [];
  ...
  return parsed
    ...
    .map((w) => {
      ...
      return {
        ...
        alpha: 1,
        bounds,
      } satisfies MacWindowInfo;
    })
    .filter((w) => Number.isFinite(w.id) && w.id > 0);
}

interface ChildBackgroundProcess {
  kind: "process";
  process: cp.ChildProcess;
  output: string;
}

interface VisibleTerminalProcess {
  kind: "terminal";
  terminal: vscode.Terminal;
  output: string;
}

type BackgroundProcess = ChildBackgroundProcess | VisibleTerminalProcess;
...
function normalizePathLikeQuery(input: string): string {
  return input
    .trim()
    .replace(/^['"`]|['"`]$/g, "")
    .replace(/\\/g, "/")
...
function isLikelyGlobPattern(query: string): boolean {
  return /[*?{}\[\]]/.test(query);
}

function toFileSearchGlobCandidates(rawQuery: string): string[] {
...
function toGrepFallbackQueries(rawQuery: string): string[] {
  const normalized = normalizePathLikeQuery(rawQuery);
  if (!normalized) {
    return [];
  }
...
function shouldSkipBulkReplacePath(filePath: string): boolean {
...
export async function tool_read_file(args: Record<string, unknown>): Promise<unknown> {
...
export async function tool_write_file(args: Record<string, unknown>): Promise<unknown> {
  const filePath = String(args.filePath ?? "");
  if (!filePath) {
    throw new Error("filePath is required.");
  }
...
export async function tool_edit_file(args: Record<string, unknown>): Promise<unknown> {
  const filePath = String(args.filePath ?? "");
  if (!filePath) {
    throw new Error("filePath is required.");
  }
...
export async function tool_list_dir(args: Record<string, unknown>): Promise<unknown> {
...
export async function tool_file_search(args: Record<string, unknown>): Promise<unknown> {
...
export async function tool_grep_search(args: Record<string, unknown>): Promise<unknown> {
  const query = String(args.query ?? "");
...
export async function tool_replace_in_files(args: Record<string, unknown>): Promise<unknown> {
...
export async function tool_run_in_terminal(args: Record<string, unknown>): Promise<unknown> {
  ...
  const violation = getTerminalPolicyViolation(command);
  if (violation) {
    return {
      explanation,
      goal,
  ...
  if (showTerminal) {
    ...
    if (isBackground) {
      const id = nextId();
      const state: VisibleTerminalProcess = {
        kind: "terminal",
        terminal,
        output: "",
      };
  ...
  const id = nextId();
  const state: ChildBackgroundProcess = {
    kind: "process",
    process: child,
    output: "",
  };
...
export async function tool_get_terminal_output(args: Record<string, unknown>): Promise<unknown> {
...
export async function tool_kill_terminal(args: Record<string, unknown>): Promise<unknown> {
  const id = String(args.id ?? "");
  if (!id) {
    throw new Error("id is required.");
  }
...
export async function tool_get_completion_checklist(
  ...
  try {
    ...
    const items: Array<{ text: string; checked: boolean; line: number }> = [];
    ...
    return {
      ...
      allComplete: true,
      message:
        "No .github/completion-checklist.md found. Create one to define acceptance criteria.",
    };
  }
}
...
function getAgentChecklistPath(): string | null {
  const workspaceFolders = vscode.workspace.workspaceFolders;
 ...
 */
export async function tool_create_agent_checklist(args: Record<string, unknown>): Promise<unknown> {
  ...
  const items = (rawItems as unknown[]).map((i) => String(i));
...
export async function tool_get_agent_checklist(_args: Record<string, unknown>): Promise<unknown> {
...
export async function tool_update_agent_checklist_item(
...
export async function tool_take_screenshot(args: Record<string, unknown>): Promise<unknown> {
      ...
      if (windowTitle) {
        ...
        try {
          ...
          try {
            ...
            return {
              success: false,
              error:
                `macOS window capture requires window enumeration tooling. ` +
        ...
        if (!best) {
          ...
          return {
            success: false,
            error:
              `No on-screen window matched "${windowTitle}". ` +
...
function parseTesseractTsv(tsv: string): Array<{
  level: number;
  page: number;
  block: number;
  par: number;
  ...
  for (const raw of lines.slice(1)) {
    ...
    const [level, page, block, par, line, word, left, top, width, height, conf, ...textParts] =
      parts;
...
export async function tool_ocr_find_text(args: Record<string, unknown>): Promise<unknown> {
  ...
  const originInput = args.origin as { x?: number; y?: number } | undefined;
  const origin = {
  ...
  const tmpPng = path.join(
    os.tmpdir(),
    `copilot-ocr-${Date.now()}-${Math.random().toString(36).slice(2, 8)}.png`,
  );
  ...
  try {
    ...
    const lineKey = (row: (typeof rows)[number]) =>
      `${row.page}:${row.block}:${row.par}:${row.line}`;
...
export async function tool_gui_key_hold(args: Record<string, unknown>): Promise<unknown> {
...
export async function tool_analyze_image(args: Record<string, unknown>): Promise<unknown> {
  ...
  const ollamaUrl = config.get<string>("endpoint", "http://localhost:11434");
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const fallbackData = (await fallbackResponse.json()) as { response: string };
...
export async function tool_gui_click(args: Record<string, unknown>): Promise<unknown> {
...
export async function tool_gui_type(args: Record<string, unknown>): Promise<unknown> {
...
export async function tool_gui_scroll(args: Record<string, unknown>): Promise<unknown> {
  const x = Number(args.x);
  const y = Number(args.y);
...
export async function tool_gui_key(args: Record<string, unknown>): Promise<unknown> {
  const key = String(args.key ?? "");
  const modifiers = (args.modifiers as string[] | undefined) ?? [];
...
export async function tool_list_windows(args: Record<string, unknown>): Promise<unknown> {
...
'@;`;
      ...
      const windows = JSON.parse(stdout.trim() || "[]") as Array<Record<string, unknown>>;
...
export async function tool_focus_window(args: Record<string, unknown>): Promise<unknown> {
...
'@;`;
    ...
    if (process.platform === "darwin") {
      ...
      try {
        const { stdout } = await execFileAsync("yabai", ["-m", "query", "--windows"]);
        const windows = JSON.parse(stdout) as Array<any>;
...
export async function tool_launch_app(args: Record<string, unknown>): Promise<unknown> {
...
export async function tool_wait_for_condition(args: Record<string, unknown>): Promise<unknown> {
  ...
  while (Date.now() - startTime < timeout) {
    ...
    switch (type) {
      case "file_exists":
        ...
        break;
      case "port_open":
        conditionMet = await new Promise<boolean>((resolve) => {
      ...
      case "screen_contains":
        try {
          ...
          const screenshot = (await tool_take_screenshot({ delay: 0 })) as any;
          if (screenshot?.success && typeof screenshot.image === "string") {
            const metaOrigin = screenshot?.meta?.origin as { x?: number; y?: number } | undefined;
            const origin = {
...
export async function tool_get_diagnostics(args: Record<string, unknown>): Promise<unknown> {
    ...
    for (const diag of diagnostics) {
      if (diag.severity <= minSeverity) {
        results.push({
          ...
          severity:
            Object.entries(severityMap).find(([, v]) => v === diag.severity)?.[0] ?? "unknown",
  ...
  return {
    ...
    summary:
      results.length === 0 ? "No diagnostics found." : `${results.length} diagnostic(s) found.`,
  };
}

export async function tool_get_workspace_symbols(args: Record<string, unknown>): Promise<unknown> {
  const query = String(args.query ?? "");
...
export async function tool_get_document_symbols(args: Record<string, unknown>): Promise<unknown> {
  const filePath = String(args.filePath ?? "");
  if (!filePath) {
...
export async function tool_get_references(args: Record<string, unknown>): Promise<unknown> {
  const filePath = String(args.filePath ?? "");
...
export async function tool_get_definition(args: Record<string, unknown>): Promise<unknown> {
  const filePath = String(args.filePath ?? "");
  const line = Number(args.line ?? 0);
  const character = Number(args.character ?? 0);
...
export async function tool_http_request(args: Record<string, unknown>): Promise<unknown> {
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// Autonomous Agent Progress Tracking
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
export async function tool_agent_progress(args: Record<string, unknown>): Promise<unknown> {
  const completed = (args.completed as string[] | undefined) ?? currentAgentState.completed;
  const remaining = (args.remaining as string[] | undefined) ?? currentAgentState.remaining;
  const status = String(args.status ?? "in_progress") as AgentState["status"];
  const blockerDescription = args.blockerDescription as string | undefined;
...
async function globalFetch(input: string, init?: RequestInit): Promise<Response> {
