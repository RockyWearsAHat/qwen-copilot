...
import { getAgentState, resetAgentState } from "../tools/handlers";
...
export interface AutonomousLoopOptions {
  ...
  maxTurns: number;
  /** Maximum consecutive turns with zero mutations before escalation */
  maxIdleTurns: number;
  ...
  /** Stream for sending markdown updates to the user */
...
export async function runAutonomousLoop(
...
): Promise<{ success: boolean; summary: string; turns: number }> {
  const { maxTurns, maxIdleTurns, userRequest, toolRegistry, output, progress, token, stream } =
    options;
  ...
  const toolSpecs: LlmToolSpec[] = allTools.map((t) => ({
    type: "function",
  ...
  while (turnCount < maxTurns) {
    ...
    const visionMessages = await prepareMessagesWithVision(
    ...
    const chatRequest: ChatRequest = {
      ...
      model,
      tools: toolSpecs,
      messages: visionMessages as LlmMessage[],
      temperature,
    };
    ...
    if (toolCalls.length === 0) {
      ...
      if (agentState.status === "completed") {
        ...
        // Verify checklist
        try {
          const checklistResult = (await toolRegistry.execute("get_completion_checklist", {})) as {
            allComplete: boolean;
            summary: string;
          };
    ...
    for (const toolCall of toolCalls) {
      ...
      output.appendLine(
        `[agent-loop] Tool: ${toolName}(${JSON.stringify(toolArgs).slice(0, 200)})`,
      );
      ...
      try {
        ...
        const toolMessage: LlmMessage = {
          ...
          tool_name: toolName,
          content:
            resultStr.length > 20000 ? resultStr.slice(0, 20000) + "\n...[truncated]" : resultStr,
        };
        ...
        if (
          ...
          toolMessage.images = [(toolResult as { image: string }).image];
        }
  ...
  return {
    success: false,
    summary: `Agent exhausted maximum turns (${maxTurns}). State: ${JSON.stringify(getAgentState())}`,
    turns: turnCount,
  };
}
...
function parseToolArgs(toolCall: ToolCall): Record<string, unknown> {
  ...
  if (typeof raw === "string") {
    try {
      ...
      console.warn(
        `[local-qwen] tool '${toolCall.function.name}' has unparseable JSON arguments — raw: ${String(raw).slice(0, 120)}`,
      );
      return {};
    }
  }
