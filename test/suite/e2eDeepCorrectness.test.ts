...
type DeepFlowMode = "provider" | "raw";
...
function repetitionEvents(text: string): number {
...
async function singleTurn(
  client: OllamaClient,
  provider: LocalLanguageModelProvider,
  flowMode: DeepFlowMode,
  endpoint: string,
  model: string,
  prompt: string,
  tools: LlmToolSpec[],
...
function toVsCodeTools(tools: LlmToolSpec[]): vscode.LanguageModelChatTool[] {
...
function scoreViteScaffold(
  ...
  if (
    !/<function\s*=|<local_qwen_tool_call>|Running\s+[a-z0-9_.-]+\./i.test(
      content,
    )
  ) {
    format += 0.25;
    ...
    notes.push(
      "response contained pseudo-tool formatting instead of native tool_calls",
    );
  }
  ...
  if (content.length <= 260) {
    ...
    notes.push(
      `response too verbose for one-step scaffold (${content.length} chars)`,
    );
  }
  ...
  if (call.function.name === "create_new_workspace") {
    ...
    if (query.includes(workspaceRoot)) {
      ...
      notes.push(
        "workspace query did not target the requested blank workspace path",
      );
    }
  ...
  if (
    ...
    notes.push(
      "terminal scaffold command did not include a valid vite command",
    );
  }
  ...
  if (
    ...
    notes.push(
      "terminal scaffold command included chained or unsafe shell actions",
    );
  }
...
function scoreCreateHtml(
  ...
  if (content.length <= 260) {
    ...
    notes.push(
      `response too verbose for one-step file creation (${content.length} chars)`,
    );
  }
  if (
    !/<function\s*=|<local_qwen_tool_call>|Running\s+[a-z0-9_.-]+\./i.test(
  ...
  if (filePath.includes(workspaceRoot) || filePath === "index.html") {
    ...
    notes.push(
      "filePath did not target requested workspace or relative index.html",
    );
  }
...
function scorePythonSnippet(
  ...
  if (content.length <= 260) {
    ...
    notes.push(
      `response too verbose for one-step snippet run (${content.length} chars)`,
    );
  }
  if (
    !/<function\s*=|<local_qwen_tool_call>|Running\s+[a-z0-9_.-]+\./i.test(
  ...
  if (call.function.name === "mcp_pylance_mcp_s_pylanceRunCodeSnippet") {
    ...
    notes.push(
      `expected pylance snippet execution tool, got ${call.function.name}`,
    );
  }
  ...
  if (/print\(2\s*\+\s*2\)/.test(codeSnippet)) {
    ...
    notes.push(
      "codeSnippet did not preserve required python expression print(2 + 2)",
    );
  }

  if (
    toolCalls.some((toolCall) => toolCall.function.name === "run_in_terminal")
  ) {
    useful = 0;
...
function scoreIntentPersistence(observation: ChatObservation): ComplianceScore {
  const notes: string[] = [];
  ...
  const replaceCalls = toolCalls.filter(
    (call) => call.function.name === "replace_in_files",
  );
  ...
  const hasTextIntentSignal =
    mappingFrom.test(lowerContent) && mappingTo.test(lowerContent);
  const duplicates = repetitionEvents(content);
  const hasPseudoFormat =
    /<function\s*=|<local_qwen_tool_call>|Running\s+[a-z0-9_.-]+\./i.test(
      content,
    );
  ...
  if (content.length <= 900 || toolCalls.length > 0) {
    ...
    notes.push(
      `excessively verbose narrative during intent flow (${content.length} chars)`,
    );
  }
  ...
  if (hasCorrectReplaceCall || hasTextIntentSignal) {
    ...
    notes.push(
      "did not preserve replacement mapping intent (/assets/level1.json -> /level1.json)",
    );
  }
  if (!invertedMapping) {
    useful += 0.4;
...
function scoreRepositoryResearch(
  observation: ChatObservation,
): ComplianceScore {
  const notes: string[] = [];
  const content = observation.content.trim();
  ...
  if (toolCalls.length === 1) {
    ...
    notes.push(
      `expected exactly one repository lookup call, got ${toolCalls.length}`,
    );
  }
  if (content.length <= 320) {
    ...
    notes.push(
      `response too verbose for repo lookup task (${content.length} chars)`,
    );
  }
  if (
    !/<function\s*=|<local_qwen_tool_call>|Running\s+[a-z0-9_.-]+\./i.test(
  ...
  if (call.function.name === "github_repo") {
    ...
    if (
      ...
      notes.push(
        "lookup query was too generic and missed requested behavior keywords",
      );
    }
...
function scoreSafeTerminalProbe(observation: ChatObservation): ComplianceScore {
  const notes: string[] = [];
  const content = observation.content.trim();
  const toolCalls = observation.toolCalls;
  ...
  if (content.length <= 260) {
    ...
    notes.push(
      `response too verbose for one-step terminal probe (${content.length} chars)`,
    );
  }
  if (
    !/<function\s*=|<local_qwen_tool_call>|Running\s+[a-z0-9_.-]+\./i.test(
  ...
  if (!call || call.function.name !== "run_in_terminal") {
    notes.push(
      `expected run_in_terminal for command probe, got ${call?.function.name ?? "none"}`,
    );
    return { format: clamp01(format), useful: 0, notes };
  }
  ...
  if (!/&&|\|\||;|\n/.test(command)) {
    ...
    notes.push(
      "terminal probe command used command chaining for a one-step request",
    );
  }
  ...
  if (!/rm\s+-rf|sudo\s+|curl\s+|wget\s+|chmod\s+|chown\s+/i.test(command)) {
    ...
    notes.push(
      "terminal probe command included destructive or unrelated operations",
    );
  }
...
function scoreMutationWithVerification(
  observation: ChatObservation,
): ComplianceScore {
  const notes: string[] = [];
  const content = observation.content.trim();
  ...
  if (toolCalls.length >= 2) {
    ...
    notes.push(
      `expected at least two tool calls (mutate + verify), got ${toolCalls.length}`,
    );
  }
  if (content.length <= 420) {
    ...
    notes.push(
      `response too verbose for mutate+verify flow (${content.length} chars)`,
    );
  }
  if (
    !/<function\s*=|<local_qwen_tool_call>|Running\s+[a-z0-9_.-]+\./i.test(
  ...
  let useful = 0;
  const hasMutation = toolCalls.some((call) =>
    mutatingNames.has(call.function.name),
  );
  const hasVerification = toolCalls.some((call) => {
    ...
    return /\b(test|lint|typecheck|types|build|compile|verify|tsc)\b/.test(
      command,
    );
  });
  ...
  if (hasVerification) {
    ...
    notes.push(
      "did not run verification after mutation (expected get_errors or test/lint/build command)",
    );
  }
...
function scoreRepoInstructionBootstrap(
  observation: ChatObservation,
): ComplianceScore {
  const notes: string[] = [];
  const content = observation.content.trim();
  ...
  if (toolCalls.length === 1) {
    ...
    notes.push(
      `expected exactly one instruction bootstrap call, got ${toolCalls.length}`,
    );
  }
  if (content.length <= 320) {
    format += 0.25;
    ...
    notes.push(
      `response too verbose for instruction bootstrap (${content.length} chars)`,
    );
  }
  if (
    !/<function\s*=|<local_qwen_tool_call>|Running\s+[a-z0-9_.-]+\./i.test(
  ...
  if (filePath.endsWith(".github/copilot-instructions.md")) {
    ...
    notes.push(
      `expected .github/copilot-instructions.md path, got '${filePath}'`,
    );
  }
...
function scoreDebugFixVerifyWorkflow(
  observation: ChatObservation,
): ComplianceScore {
  const notes: string[] = [];
  const content = observation.content.trim();
  ...
  if (toolCalls.length >= 3) {
    ...
    notes.push(
      `expected at least three calls (diagnose → mutate → verify), got ${toolCalls.length}`,
    );
  }
  if (content.length <= 520) {
    ...
    notes.push(
      `response too verbose for debug-fix-verify workflow (${content.length} chars)`,
    );
  }
  if (
    !/<function\s*=|<local_qwen_tool_call>|Running\s+[a-z0-9_.-]+\./i.test(
  ...
  const firstDiagnosisIndex = calls.findIndex(
    (call) =>
  ...
  const firstMutationIndex = calls.findIndex((call) =>
    mutatingNames.has(call.name),
  );
  const verifyAfterMutationIndex = calls.findIndex(
    (call, index) =>
      index > firstMutationIndex &&
      (call.name === "get_errors" ||
        (call.name === "run_in_terminal" &&
          /\b(test|lint|typecheck|types|build|compile|verify|tsc)\b/.test(
            String(call.args.command ?? "").toLowerCase(),
          ))),
  );
...
function scoreReadContextOneStep(
  observation: ChatObservation,
): ComplianceScore {
  const notes: string[] = [];
  const content = observation.content.trim();
  ...
  if (content.length <= 300) {
    ...
    notes.push(
      `response too verbose for one-step read context (${content.length} chars)`,
    );
  }
  if (
    !/<function\s*=|<local_qwen_tool_call>|Running\s+[a-z0-9_.-]+\./i.test(
  ...
  if (
    ...
    notes.push(
      `expected broad requested line range (1-120 style), got ${startLine}-${endLine}`,
    );
  }
  if (endLine > startLine) {
    ...
    notes.push(
      "endLine must be greater than startLine for meaningful context read",
    );
  }
...
function scorePatchEditDiscipline(
  observation: ChatObservation,
): ComplianceScore {
  const notes: string[] = [];
  const content = observation.content.trim();
  ...
  if (content.length <= 340) {
    ...
    notes.push(
      `response too verbose for patch discipline case (${content.length} chars)`,
    );
  }
  if (
    !/<function\s*=|<local_qwen_tool_call>|Running\s+[a-z0-9_.-]+\./i.test(
  ...
  if (
    patchInput.includes("*** Begin Patch") &&
  ...
  if (!toolCalls.some((entry) => entry.function.name === "run_in_terminal")) {
    ...
    notes.push(
      "patch-discipline case should not use terminal for simple README edit",
    );
  }
...
function scoreVscodeRepoResearch(
  observation: ChatObservation,
): ComplianceScore {
  const notes: string[] = [];
  const content = observation.content.trim();
  ...
  if (content.length <= 360) {
    ...
    notes.push(
      `response too verbose for vscode repo lookup (${content.length} chars)`,
    );
  }
  if (
    !/<function\s*=|<local_qwen_tool_call>|Running\s+[a-z0-9_.-]+\./i.test(
...
function scoreSafeGitProbe(observation: ChatObservation): ComplianceScore {
  const notes: string[] = [];
  const content = observation.content.trim();
  const toolCalls = observation.toolCalls;
  ...
  if (!call || call.function.name !== "run_in_terminal") {
    notes.push(
      `expected run_in_terminal, got ${call?.function.name ?? "none"}`,
    );
    return { format: clamp01(format), useful: 0, notes };
  }
  ...
  if (!/rm\s+-rf|sudo\s+|curl\s+|wget\s+|chmod\s+|chown\s+/i.test(command)) {
    ...
    notes.push(
      "git probe command included destructive or unrelated operations",
    );
  }
...
function scoreReadmeOnlyFixWorkflow(
  observation: ChatObservation,
): ComplianceScore {
  const notes: string[] = [];
  const content = observation.content.trim();
  ...
  if (toolCalls.length >= 2) {
    ...
    notes.push(
      `expected at least two calls (fix + verify), got ${toolCalls.length}`,
    );
  }
  if (content.length <= 500) {
    ...
    notes.push(
      `response too verbose for README-only fix workflow (${content.length} chars)`,
    );
  }
  if (
    !/<function\s*=|<local_qwen_tool_call>|Running\s+[a-z0-9_.-]+\./i.test(
  ...
  if (patchCalls.length > 0) {
    ...
    const allReadmeOnly = patchCalls.every((call) => {
      ...
      return (
        /\*\*\* Update File: .*README\.md/i.test(patchInput) &&
...
suite("E2E deep correctness (format + useful compliance, opt-in)", function () {
  ...
  const flowMode = (
    String(process.env.LOCAL_QWEN_E2E_DEEP_FLOW ?? "provider").toLowerCase() ===
    "raw"
      ? "raw"
      : "provider"
  ) as DeepFlowMode;

  const minFormat = Math.min(
    1,
    Math.max(0, toNumber(process.env.LOCAL_QWEN_E2E_DEEP_MIN_FORMAT, 0.9)),
  );
  const minUseful = Math.min(
    1,
    Math.max(0, toNumber(process.env.LOCAL_QWEN_E2E_DEEP_MIN_USEFUL, 0.9)),
  );
  const minOverall = Math.min(
    1,
    Math.max(0, toNumber(process.env.LOCAL_QWEN_E2E_DEEP_MIN_OVERALL, 0.9)),
  );
  const minCaseUseful = Math.min(
    1,
    Math.max(
      0,
      toNumber(process.env.LOCAL_QWEN_E2E_DEEP_MIN_CASE_USEFUL, 0.75),
    ),
  );

  const client = new OllamaClient();
  const provider = new LocalLanguageModelProvider({
    appendLine: () => {},
  } as unknown as vscode.OutputChannel);
  let blankWorkspace = "";

  const oneStepTools: LlmToolSpec[] = [
    {
      type: "function",
      function: {
  ...
  const repoResearchTools: LlmToolSpec[] = [
    {
      type: "function",
      function: {
        name: "github_repo",
        description:
          "Search a specific GitHub repository for relevant code snippets.",
        parameters: {
          type: "object",
          properties: {
            repo: { type: "string" },
  ...
  const safeTerminalTools: LlmToolSpec[] = [
    {
      type: "function",
      function: {
        name: "run_in_terminal",
  ...
  const mutationVerificationTools: LlmToolSpec[] = [
    {
      type: "function",
      function: {
        name: "replace_in_files",
  ...
  const instructionBootstrapTools: LlmToolSpec[] = [
    {
      type: "function",
      function: {
        name: "create_file",
  ...
  const debugFixVerifyTools: LlmToolSpec[] = [
    {
      type: "function",
      function: {
        name: "get_errors",
  ...
  const readContextTools: LlmToolSpec[] = [
    {
      type: "function",
      function: {
        name: "read_file",
  ...
  const patchDisciplineTools: LlmToolSpec[] = [
    {
      type: "function",
      function: {
        name: "apply_patch",
  ...
  const vscodeRepoResearchTools: LlmToolSpec[] = [
    {
      type: "function",
      function: {
        name: "github_repo",
  ...
  const safeGitProbeTools: LlmToolSpec[] = [
    {
      type: "function",
      function: {
        name: "run_in_terminal",
  ...
  const readmeOnlyFixTools: LlmToolSpec[] = [
    {
      type: "function",
      function: {
        name: "apply_patch",
  ...
  setup(async () => {
    blankWorkspace = await fs.mkdtemp(
      path.join(os.tmpdir(), "local-qwen-deep-correctness-"),
    );
  });

  (runE2E ? test : test.skip)(
    "format compliance and useful result compliance both satisfy strict thresholds",
    async () => {
      const cases: Array<{ name: string; score: ComplianceScore }> = [];

      const scaffoldObservation = await singleTurn(
        client,
        provider,
        flowMode,
        endpoint,
        model,
        `In blank workspace '${blankWorkspace}', start a Vite project with exactly one tool call.`,
        oneStepTools,
      );
      cases.push({
        name: "vite-scaffold",
      ...
      const createHtmlObservation = await singleTurn(
        client,
        provider,
        flowMode,
        endpoint,
      ...
      const pythonObservation = await singleTurn(
        client,
        provider,
        flowMode,
        endpoint,
      ...
      const intentObservation = await streamTurn(
        client,
        provider,
        flowMode,
        endpoint,
        ...
        [
          {
            role: "system",
            content:
              "You are a coding agent. Keep the original replacement mapping stable through noise and corrections. Never invert from/to.",
          },
          {
            role: "user",
            content:
          ...
          },
          {
            role: "assistant",
            content:
              "Running read_file. Running grep_search. I found unrelated files and will keep exploring.",
          },
          {
            role: "user",
            content:
              "Noise: output chunks from unrelated files. Do not drift from the original mapping.",
          },
          {
            role: "user",
            content:
              "Continue the same replacement intent and finish efficiently with minimal repetition.",
          },
        ],
        intentTools,
      );
      ...
      const repoResearchObservation = await singleTurn(
        client,
        provider,
        flowMode,
        endpoint,
      ...
      const safeTerminalObservation = await singleTurn(
        client,
        provider,
        flowMode,
        endpoint,
      ...
      const mutationVerificationObservation = await singleTurn(
        client,
        provider,
        flowMode,
        endpoint,
      ...
      const instructionBootstrapObservation = await singleTurn(
        client,
        provider,
        flowMode,
        endpoint,
      ...
      const debugFixVerifyObservation = await singleTurn(
        client,
        provider,
        flowMode,
        endpoint,
      ...
      const readContextObservation = await singleTurn(
        client,
        provider,
        flowMode,
        endpoint,
      ...
      const patchDisciplineObservation = await singleTurn(
        client,
        provider,
        flowMode,
        endpoint,
      ...
      const vscodeRepoObservation = await singleTurn(
        client,
        provider,
        flowMode,
        endpoint,
      ...
      const safeGitProbeObservation = await singleTurn(
        client,
        provider,
        flowMode,
        endpoint,
      ...
      const readmeOnlyFixObservation = await singleTurn(
        client,
        provider,
        flowMode,
        endpoint,
      ...
      const formatAverage =
        cases.reduce((sum, entry) => sum + entry.score.format, 0) /
        cases.length;
      const usefulAverage =
        cases.reduce((sum, entry) => sum + entry.score.useful, 0) /
        cases.length;
      ...
      const failingCases = cases
        .filter(
          (entry) =>
            entry.score.format < minFormat ||
            entry.score.useful < minUseful ||
            entry.score.useful < minCaseUseful,
        )
        .map(
          (entry) =>
            `${entry.name}: format=${entry.score.format.toFixed(2)} useful=${entry.score.useful.toFixed(2)} notes=${entry.score.notes.join(" | ")}`,
        );
